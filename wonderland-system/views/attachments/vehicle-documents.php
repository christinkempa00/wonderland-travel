    ...
    <div class="row">
        <div class="col-lg-8">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="<?= url('/attachment-order/' . $order['id']) ?>" class="btn btn-outline-secondary">
                    <i class="fas fa-arrow-left"></i> Kembali
                </a>
            </div>
            
            <form method="POST" action="<?= url('/attachment-order/' . $order['id'] . '/vehicle-documents') ?>" id="vehicleDocumentsForm">
                <?= csrfField() ?>
        ...
        <div class="col-lg-4">
            ...
            <div class="glass-card mb-4 preview-card">
                ...
                <a href="<?= url('/attachment-order/' . $order['id'] . '/vehicle-documents-print') ?>" target="_blank" class="btn btn-danger btn-block">
            ...
            <div class="glass-card">
                ...
                <div class="quick-actions" style="display:flex;flex-direction:column;gap:0.5rem;">
                    <?php if (!empty($documents)): ?>
                    <a href="<?= url('/attachment-order/' . $order['id'] . '/vehicle-documents-print') ?>" target="_blank" class="btn btn-danger btn-sm">
                        <i class="fas fa-print"></i> Cetak Lampiran
                    </a>
                    <?php endif; ?>
                    <a href="<?= url('/attachment-order/' . $order['id']) ?>" class="btn btn-outline-secondary btn-sm">
                        <i class="fas fa-arrow-left"></i> Kembali ke Detail
                    </a>
                    <a href="<?= url('/attachment-dashboard') ?>" class="btn btn-outline-primary btn-sm">
                        <i class="fas fa-home"></i> Dashboard Lampiran
                    </a>
                </div>
            </div>
        </div>
...
function uploadVehiclePhoto(itemId, vehicleIdx, photoType, input) {
    ...
    // Upload
    fetch(`/attachment-order/${orderId}/upload-vehicle-photo`, {
        method: 'POST',
        body: formData,
        headers: {
...
function deleteVehiclePhoto(itemId, vehicleIdx, photoType) {
    ...
    // Delete from server
    fetch(`/attachment-order/${orderId}/delete-vehicle-photo`, {
        method: 'POST',
        headers: {
